...
class SearchViewController: UIViewController, MGLMapViewDelegate, CLLocationManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        customItemViewSize = CGRect(x: 0, y: (mapView.bounds.height * 3 / 4)-49, width: view.bounds.width, height: view.bounds.height / 4)
    ...
    func mapView(_ mapView: MGLMapView, didFinishLoading style: MGLStyle) {
        DispatchQueue.global().async {
            ...
            let urlFirebase = self.destinationFileUrl
            print("Estoy en el async")
            var data:Data 
            do{
                data = try Data(contentsOf: urlFirebase)
            } catch {
                print("No se puedo obtener de firebase")
    ...
    func getRoute(from origin: CLLocationCoordinate2D,
        ...
        _ = Directions.shared.calculate(options) { (waypoints, routes, error) in
            ...
            let formattedDistance = distanceFormatter.string(fromValue: route.distance, unit: LengthFormatter.Unit.kilometer)
    ...
    func generateItemPages(feature: MGLPointFeature) {
        ...
        if let themeColor = viewControllerTheme?.themeColor, let iconImage = viewControllerTheme?.defaultMarker {
            ...
            itemView.parentController = self
...
class CustomItemView : UIView {
    ...
    var parentController: SearchViewController?
    @IBOutlet var containerView: CustomItemView!
    @IBOutlet weak var itemNameLabel: UILabel!
    @IBOutlet weak var itemAvailableLabel: UILabel!
    @IBOutlet weak var hoursLabel: UILabel!
    @IBOutlet weak var itemDescriptionLabel: UILabel!
    @IBOutlet weak var itemFareLabel: UILabel!
    @IBOutlet weak var phoneNumberLabel: UILabel!
    var iconImage = UIImage()
    @IBOutlet weak var iconImageView: UIImageView!
    @IBOutlet weak var headerView: UIView!
    @IBAction func verLugar(_ sender: UIButton) {
        var topVC = UIApplication.shared.keyWindow?.rootViewController
        while ((topVC!.presentedViewController) != nil){
            topVC = topVC!.presentedViewController
        }
        let storyBoard = UIStoryboard(name: "ReservarCasillero", bundle: nil)
        let VCReservar: ReservarCasilleroControllerViewController = storyBoard.instantiateViewController(withIdentifier: "ReservarCasillero") as! ReservarCasilleroControllerViewController
        VCReservar.place = selectedFeature.attribute(forKey: "name") as! String
        print(NSStringFromClass((UIApplication.shared.keyWindow?.rootViewController?.classForCoder)!))
        print(NSStringFromClass((topVC?.classForCoder)!))
        print(NSStringFromClass((parentController?.navigationController?.classForCoder)!))
        parentController?.navigationController?.pushViewController(VCReservar, animated: true)
        //let navigation = UINavigationController(rootViewController: topVC!)
        //topVC?.present(navigation,animated: true,completion: nil)
        //topVC?.navigationController?.pushViewController(VCReservar, animated: true)
    }
    ...
    public func updateLabels() {
        ...
        if let hours : String = selectedFeature.attribute(forKey: "hours") as? String {
            containerView.itemAvailableLabel.text = hours
        }
        ...
        let minFare : String = selectedFeature.attribute(forKey: "minFare") as? String ?? "No disponible"
        if let maxFare : String = selectedFeature.attribute(forKey: "maxFare") as? String  {
            containerView.itemFareLabel.text = "\(minFare) - \(maxFare)"
        }
    }
    ...
    func createItemView() {
        ...
        containerView.itemAvailableLabel.textColor = themeColor.lowerCardTextColor
        
        //Create phone number label.
        containerView.itemFareLabel.textColor = themeColor.lowerCardTextColor
